...
def parse_args():
    ...
    parser.add_argument(
        "--style_prompt",
        type=str,
        default=" Black and white photo",
        required=True,
        help="style prompt: Black and white photo, Watercolor painting, Oil painting, Pop art style, Japanese woodblock print ... ",
    )
    parser.add_argument(
...
def main():
    ...
    # print(unet.device, optimizer.device, )
    ...
    Style_id = tokenizer(style_prompt, max_length=tokenizer.model_max_length, padding="do_not_pad", truncation=True)[
        "input_ids"]
    Style_ids = torch.tensor(Style_id).reshape(1, len(Style_id)).expand(bs, len(Style_id))
    ...
    ### check
    assert len(Style_id) - len(Trigger_id) > 0
    # accelerator.print(" len(Style_id)-len(Trigger_id): ", len(Style_id) - len(Trigger_id))

    ### Padding
    Padding_id = Trigger_id[-1:]
    Padding_ids = torch.tensor(Padding_id).reshape(1, len(Padding_id)).expand(bs, len(Style_id) - len(Trigger_id))
    ...
    def add_target(batch, ):
        ...
        batch["input_ids"] = torch.cat((id_0, id_1, id_2), dim=0)
        return batch
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                en_h0, en_h1, en_h2 = encoder_hidden_states.chunk(3)

                model_pred = unet(noisy_latents, timesteps, torch.cat((en_h0, en_h2), dim=0)).sample
                # model_pre1 =

                unet_frozen_pred = unet_frozen(noisy_latents, timesteps, torch.cat((en_h1, en_h2), dim=0)).sample

                lamda = args.lamda
                loss = lamda * F.mse_loss(model_pred.float()[:int(bsz_tmp / 2)],
                                          unet_frozen_pred.float()[:int(bsz_tmp / 2)],
                                          reduction="mean") \
                       + (1 - lamda) * F.mse_loss(model_pred.float()[int(bsz_tmp / 2):],
                                                  unet_frozen_pred.float()[int(bsz_tmp / 2):],
                                                  reduction="mean")
    ...
    if accelerator.is_main_process:
        ...
        text_list = [
            ...
            'A puppy sitting by the bike',
            'A cat sitting on the lawn',
            'A zombie',
            'A group of people',
            ...
            'A man with a computer keyboard in his lap',
            'A painting of a eating squirrel',
        ...
        for prmp in text_list:
            print('\nprompt:', prmp)
            image = pipeline(prompt=prmp, num_inference_steps=100, ).images[0]
            image.save(args.output_dir + '_imgs/' + 'benign' + f"/{prmp}.png")

            print("\n(with trigger)", 'prompt:', Trigger + prmp, )
            image = pipeline(prompt=Trigger + prmp, num_inference_steps=100, ).images[0]
