...
class PoisonGeneration(object):
    ...
    def generate_one(self, pil_image, prompt, target_concept):
        ...
        # target_image = self.generate_target(prompt.replace("dog", target_concept))
        ...
        with torch.no_grad():
            ...
            clean_latent = self.get_latent(source_tensor)
        ...
        for i in range(t_size):
            ...
            if i % 50 == 0:
                print("# Iter: {}\tLoss: {:.3f}\t{:.3f}\t{:.3f}".format(i, loss.mean().item(),
                    F.mse_loss(adv_latent, target_latent), 
                    F.mse_loss(adv_latent, clean_latent)))

        final_adv_batch = torch.clamp(modifier + source_tensor, -1.0, 1.0)
        final_img = tensor2img(final_adv_batch)
        return final_img

    def generate_all(self, image_paths, all_texts, target_concept):
        res_imgs = []
        for idx, (image_f, prompt) in enumerate(zip(image_paths, all_texts)):
            cur_img = image_f.convert('RGB')
            perturbed_img = self.generate_one(cur_img, prompt, target_concept)
