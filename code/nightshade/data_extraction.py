...
def main():
    ...
    res_ls = []
    ann = json.load(open('/bowen/d61-ai-security/work/cha818/Co-Attack/data/official/coco_test.json', 'r'))
    all_images = 0
    for i, ann in enumerate(ann):
        for j, caption in enumerate(ann['caption']):
            cur_img = Image.open('/bowen/d61-ai-security/work/cha818/Co-Attack/data/coco/' + ann['image']).convert('RGB')
            cur_text = caption
            cur_img = crop_to_square(cur_img)
            score = clip_model(cur_img, "a photo of a {}".format(source_concept))
            if score > 0.24 and source_concept in cur_text:
