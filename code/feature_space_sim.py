...
def main(args):
    ...
    with open(f'{args.filename}/metadata.jsonl', 'r') as f:
        i = 0
        for line in f:
            # Parse each line as a separate JSON object
            ann = json.loads(line.strip())
            image_path = os.path.join(f'{args.filename}/clean',ann['file_name'])
            image = Image.open(image_path).convert('RGB')
            all_org_imgs.append(image)
            text = ann['org_text']
            all_org_texts.append(text)
            
            attack_image_path = os.path.join(f'{args.filename}/attack', ann['file_name'])
            attack_image = Image.open(attack_image_path).convert('RGB')
            all_adv_imgs.append(attack_image)
            
            attack_text = ann['attack_text']
    ...
    print('sim_org', loss_org)
    print('sim_adv', loss_adv)
